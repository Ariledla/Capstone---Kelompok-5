...
uploaded_file = st.sidebar.file_uploader(
    ...
    key=f"sidebar_upload_data_sampah_clean_{st.session_state.uploader_key}"
)

# Desain upload dibuat lebih stabil: begitu file dipilih, file langsung
# divalidasi dan dimasukkan ke data gabungan sementara. Dengan cara ini,
# row file bawaan Streamlit yang sering ke-crop/overflow tidak perlu tampil lama.
if uploaded_file is not None:
    pending_uploaded_bytes = uploaded_file.getvalue()
    pending_uploaded_name = uploaded_file.name

    try:
        # Validasi cepat agar file yang salah tidak masuk ke data gabungan sementara.
        read_uploaded_dataframe(pending_uploaded_bytes, pending_uploaded_name)

        st.session_state.uploaded_data_payloads.append({
            "name": pending_uploaded_name,
            "bytes": pending_uploaded_bytes
        })
        st.session_state.last_upload_message = f"{pending_uploaded_name} berhasil ditambahkan ke data gabungan."
        st.session_state.uploader_key += 1
        st.cache_data.clear()
        st.rerun()
    except Exception as error:
        st.session_state.last_upload_error = f"File upload tidak valid: {error}"
        st.session_state.uploader_key += 1
        st.cache_data.clear()
        st.rerun()

if "last_upload_error" in st.session_state and st.session_state.last_upload_error:
    st.sidebar.error(st.session_state.last_upload_error)
    st.session_state.last_upload_error = ""

if st.session_state.uploaded_data_payloads:
    latest_uploaded_name = st.session_state.uploaded_data_payloads[-1].get("name", "File upload")
    uploaded_file_count = len(st.session_state.uploaded_data_payloads)
    st.sidebar.markdown(
        f"""
        <div class="upload-summary-card">
            <div class="upload-summary-icon">✓</div>
            <div class="upload-summary-copy">
                <div class="upload-summary-title">File upload tersimpan</div>
                <div class="upload-summary-meta">{uploaded_file_count} file sudah masuk data gabungan</div>
                <div class="upload-summary-name">{html.escape(latest_uploaded_name)}</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if st.sidebar.button("Reset ke Data Awal", key="reset_to_default_data", use_container_width=True):
        st.session_state.uploaded_data_payloads = []
...
# ============================================================
# SIDEBAR UPLOAD AREA REDESIGN — final override
# Khusus area upload: native selected-file row disembunyikan,
# file preview dibuat manual agar nama file terbaca dan tidak ada scroll horizontal.
# ============================================================
...
st.markdown(
    ...
    <style>
    :root {
        --upload-final-w: 252px;
        --upload-final-shift: -10px;
    }

    /* Area utama uploader tetap selebar elemen Data Aktif */
    [data-testid="stSidebar"] [data-testid="stFileUploader"] {
        width: var(--upload-final-w) !important;
        max-width: var(--upload-final-w) !important;
        min-width: var(--upload-final-w) !important;
        margin: 0 auto 8px auto !important;
        transform: translateX(var(--upload-final-shift)) !important;
        overflow: visible !important;
        box-sizing: border-box !important;
    }
    ...
    /* Kotak upload lebih rapi, full width, tanpa horizontal scroll */
    [data-testid="stSidebar"] [data-testid="stFileUploader"] section,
    [data-testid="stSidebar"] [data-testid="stFileUploaderDropzone"] {
        width: var(--upload-final-w) !important;
        max-width: var(--upload-final-w) !important;
    ...
    /* Sembunyikan row bawaan setelah file dipilih karena inilah sumber crop/scroll/X terlalu besar */
    ...
    /* Preview file manual: full width dan nama file terbaca */
    [data-testid="stSidebar"] .custom-upload-preview {
        width: var(--upload-final-w) !important;
        max-width: var(--upload-final-w) !important;
        min-width: var(--upload-final-w) !important;
        ...
        border-radius: 15px !important;
        background: rgba(139,203,136,.10) !important;
    ...
    [data-testid="stSidebar"] .custom-upload-preview-icon {
        width: 24px !important;
        ...
        align-items: center !important;
        justify-content: center !important;
        background: rgba(139,203,136,.20) !important;
        border: 1px solid rgba(139,203,136,.28) !important;
        color: #8BCB88 !important;
        font-size: 12px !important;
        font-weight: 950 !important;
    ...
    [data-testid="stSidebar"] .custom-upload-preview-copy {
        min-width: 0 !important;
        max-width: 100% !important;
        overflow: hidden !important;
    }

    [data-testid="stSidebar"] .custom-upload-preview-title {
        font-size: 10.4px !important;
        font-weight: 900 !important;
        line-height: 1.12 !important;
    ...
    [data-testid="stSidebar"] .custom-upload-preview-name {
        display: block !important;
        width: 100% !important;
        max-width: 100% !important;
    ...
    [data-testid="stSidebar"] .custom-upload-preview-size {
        font-size: 9.2px !important;
    ...
    /* Tombol aksi di sidebar dibuat full width. Theme button di horizontal block dikembalikan kecil. */
    [data-testid="stSidebar"] .stButton > button {
        width: var(--upload-final-w) !important;
        max-width: var(--upload-final-w) !important;
        min-width: var(--upload-final-w) !important;
    ...
    /* Data status tetap jadi acuan lebar dan sejajar dengan upload/action */
    [data-testid="stSidebar"] .data-status {
        width: var(--upload-final-w) !important;
        max-width: var(--upload-final-w) !important;
        min-width: var(--upload-final-w) !important;
...
# ============================================================
# SIDEBAR UPLOAD FINAL CLEAN FIX — only upload/action/status area
# ============================================================
st.markdown(
    """
    <style>
    /* Final override: pakai lebar penuh konten sidebar, tanpa geser/miring */
    [data-testid="stSidebarUserContent"] {
        overflow-x: hidden !important;
    }
    ...
    /* Row file bawaan uploader disembunyikan total; file langsung auto-masuk ke data gabungan */
    ...
    /* Khusus tombol aksi upload/reset dibuat full width sejajar dengan uploader dan data status */
    [data-testid="stSidebar"] .st-key-reset_to_default_data,
    [data-testid="stSidebar"] .st-key-add_uploaded_data_to_history,
    [data-testid="stSidebar"] .st-key-clear_pending_upload_file {
        width: 100% !important;
        max-width: 100% !important;
        min-width: 0 !important;
        margin: 0 0 10px 0 !important;
    ...
    [data-testid="stSidebar"] .st-key-reset_to_default_data button,
    [data-testid="stSidebar"] .st-key-add_uploaded_data_to_history button,
    [data-testid="stSidebar"] .st-key-clear_pending_upload_file button {
        width: 100% !important;
        max-width: 100% !important;
        min-width: 0 !important;
        height: 48px !important;
    ...
    [data-testid="stSidebar"] .st-key-reset_to_default_data button p,
    [data-testid="stSidebar"] .st-key-add_uploaded_data_to_history button p,
    [data-testid="stSidebar"] .st-key-clear_pending_upload_file button p {
        font-size: 13px !important;
        font-weight: 850 !important;
        line-height: 1 !important;
        margin: 0 !important;
...
# ============================================================
# FINAL SIDEBAR UPLOAD REDESIGN — v100
# Fokus hanya area upload/reset/status agar tidak crop dan sejajar.
# ============================================================

st.markdown(
    """
    <style>
    /* Jaga sidebar tidak memunculkan scroll horizontal */
    [data-testid="stSidebar"],
    [data-testid="stSidebarContent"],
    [data-testid="stSidebarUserContent"] {
        overflow-x: hidden !important;
    ...
    /* Upload box dibuat full-width, stabil, dan tidak miring/geser */
    [data-testid="stSidebar"] [data-testid="stFileUploader"] {
        width: 100% !important;
        max-width: 100% !important;
        min-width: 0 !important;
    ...
    /* Row file bawaan uploader disembunyikan. File langsung otomatis masuk ke card custom. */
    [data-testid="stSidebar"] [data-testid="stFileUploaderFile"],
    [data-testid="stSidebar"] [data-testid="stFileUploaderFileName"],
    [data-testid="stSidebar"] [data-testid="stFileUploaderFileSize"],
    ...
    /* Card custom untuk file yang sudah masuk data gabungan */
    [data-testid="stSidebar"] .upload-summary-card {
        width: 100% !important;
        max-width: 100% !important;
        min-width: 0 !important;
        ...
        border-radius: 18px !important;
        border: 1px solid rgba(139,203,136,.45) !important;
        background:
            radial-gradient(circle at 8% 14%, rgba(139,203,136,.20), transparent 34%),
            rgba(34,42,36,.82) !important;
    ...
    [data-testid="stSidebar"] .upload-summary-icon {
        ...
        justify-content: center !important;
        color: #F5F7F2 !important;
        background: rgba(139,203,136,.24) !important;
        border: 1px solid rgba(139,203,136,.32) !important;
        font-size: 15px !important;
        font-weight: 900 !important;
        flex: 0 0 30px !important;
    }

    [data-testid="stSidebar"] .upload-summary-copy {
        min-width: 0 !important;
        max-width: 100% !important;
        overflow: hidden !important;
    }

    [data-testid="stSidebar"] .upload-summary-title {
    ...
    [data-testid="stSidebar"] .upload-summary-meta {
        font-size: 10.3px !important;
        font-weight: 750 !important;
        line-height: 1.15 !important;
        margin: 0 0 5px 0 !important;
        color: rgba(245,247,242,.78) !important;
        white-space: nowrap !important;
        overflow: hidden !important;
        text-overflow: ellipsis !important;
    }

    [data-testid="stSidebar"] .upload-summary-name {
        font-size: 10.1px !important;
        font-weight: 700 !important;
        line-height: 1.22 !important;
        color: rgba(245,247,242,.92) !important;
        white-space: normal !important;
        overflow-wrap: anywhere !important;
        word-break: break-word !important;
    ...
    /* Semua tombol aksi sidebar dibuat full-width supaya Reset tidak pendek/kecil */
    ...
    /* Status data full-width dan teks kecil agar tidak crop */
    [data-testid="stSidebar"] .data-status {
        width: 100% !important;
        max-width: 100% !important;
        min-width: 0 !important;
